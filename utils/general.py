...
def soft_nms(boxes, scores, sigma=0.5, Nt=0.3, threshold=0.001, method=2):
    """
    Performs soft non-maximum suppression (Soft-NMS) on bounding boxes.
    Args:
        boxes (np.array): Array of bounding boxes with shape [N, 4].
        scores (np.array): Array of scores with shape [N,].
        sigma (float): Gaussian function parameter.
        Nt (float): IOU threshold.
        threshold (float): Score threshold.
        method (int): 1 for linear, 2 for Gaussian.
    Returns:
        np.array: The indexes of the kept boxes.
    """
    N = boxes.shape[0]
    indexes = np.arange(N)
    for i in range(N):
        ...
        indexes[i], indexes[maxpos] = indexes[maxpos], indexes[i]
...
def soft_non_max_suppression(
    ...
    max_time_img = 5.0  # seconds to run NMS per image
    time_limit = 60.0  # seconds to run NMS per batch
    ...
    for xi, x in enumerate(prediction):  # image index, image inference
        ...
        keep = soft_nms(boxes_np, scores_np)
        x = x[keep]

        if merge and (1 < n < 3000):
            iou = box_iou(boxes[keep], boxes) > iou_thres  # iou matrix
            weights = iou * scores[None]  # box weights
            x[:, :4] = torch.mm(weights, x[:, :4]).float() / weights.sum(1, keepdim=True)  # merged boxes
            if redundant:
                keep = x[:, 4].argsort(descending=True)[:max_det]  # sort by confidence
            else:
                keep = x[:, 4].argsort(descending=True)[:max_det]  # sort by confidence
            x = x[keep]
